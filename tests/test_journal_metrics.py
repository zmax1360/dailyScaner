...
def test_live_journal_concat_fifo_expected_totals():
    ...
    assert stats["wins"] >= 159, (
        f"wins dropped to {stats['wins']}; journal may have been truncated "
        "(see data/journal rebuild incident 2026-09-12)"
    )
    # Dated snapshot: 58-session journal as of 2026-09-12. Update as sessions accrue.
    assert stats["total_realized_pnl"] == pytest.approx(-1798.31, abs=0.01)
